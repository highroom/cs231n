...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        # shape (N, C).                                                             #
        #############################################################################
        h1 = np.maximum(0, np.dot(X, W1) + b1)  # X(N, D), W1(D, H) => h1(N, H)   h1: hidden layer
        scores = np.dot(h1, W2) + b2    # scores(N, C)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        # classifier loss.                                                          #
        #############################################################################
        exp_class_score = np.exp(scores)  # scores(N, C)
        ...
        dh2[np.arange(N), y] -= 1  # dz = y^ - y
        dh2 /= N  # dh2(N, C)

        dW2 = np.dot(h1.T, dh2)  # h1.T(H, N)  # dh2(N, C)
        dW2 += 2 * reg * W2      # dW2(H, C)

        db2 = np.sum(dh2, axis=0)  # db2(C,)

        # layer 1
        dh1 = np.dot(dh2, W2.T)   # dh1(N, H)
        dh1[h1 <= 0] = 0

        dW1 = np.dot(X.T, dh1)   # dW1 (D, H)
        dW1 += 2 * reg * W1
