...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # of 0.5 tonnnnnn simplify the expression for the gradient.                      #
        ############################################################################
        ...
        grads['W1'] = dW1 + self.reg * self.params['W1']  # reg*W 是正则项求导的结果
        grads['b1'] = db1
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            self.params['W%s' % (layer + 1)] = np.random.normal(0, weight_scale, size=layer_dim)
            self.params['b%s' % (layer + 1)] = np.zeros(layer_dim[1])
            if self.normalization and layer != self.num_layers - 1:
                self.params['gama%s' % (layer + 1)] = np.ones(layer_dim[1])
                self.params['beta%s' % (layer + 1)] = np.zeros(layer_dim[1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == 'batchnorm':
                scores, bn_cache = batchnorm_forward(scores, self.params['gama%s' % (i + 1)],
                                                     self.params['beta%s' % (i + 1)], self.bn_params[i])
                cache.append(bn_cache)
            elif self.normalization == 'layernorm':
                scores, ln_cache = layernorm_forward(scores, self.params['gama%s' % (i + 1)],
                                                     self.params['beta%s' % (i + 1)], self.bn_params[i])
