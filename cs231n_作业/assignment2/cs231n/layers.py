...
def relu_backward(dout, cache):
    ...
    Returns:
    - dx: Gradient with respect to x   (N, d_1, ... d_k)
    """
    dx, x = None, cache
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = [(0, 0), (0, 0)]
    pad.extend(padding)  # pad=[(0,0),(0,0),(padding, padding), (padding, padding)] 只有后面2个维度长和宽需要padding
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(n):  # 每个样本点
        for j in range(f):  # 每个filter
            for _w in range(out_w):  # 水平方向
                for _h in range(out_h):  # 竖直方向
                    dw[j] += dout[i, j, _h, _w] * x_pad[i, :, _h * stride[1]: _h * stride[1] + ww,
                                                  _w * stride[0]: _w * stride[0] + hh]
                    db[j] += dout[i, j, _h, _w]
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_hat = x_hat.T
    d = x_hat.shape[0]
    dx = 1. / d * inv_sigma * (d * dxhat - np.sum(dxhat, axis=0) - x_hat * np.sum(dxhat * x_hat, axis=0))
    dx = dx.T
...
def softmax_loss(x, y):
    ...
    Z = np.sum(np.exp(shifted_logits), axis=1, keepdims=True)  # 减去最大值是为了防止e的1000次方结果为inf.
    log_probs = shifted_logits - np.log(Z)  # 下面2步实现了np.exp(x)/sum(np.exp(x))
    probs = np.exp(log_probs)
    N = x.shape[0]
    loss = -np.sum(log_probs[np.arange(N), y]) / N
    dx = probs.copy()
    dx[np.arange(N), y] -= 1  # dz = yhat - y
    dx /= N
    return loss, dx
